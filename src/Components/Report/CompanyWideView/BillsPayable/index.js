...
import { payablesBillsFn } from "Services/Report"
...
const BillsPayable = () => {
  ...
  const { data, isLoading } = useQuery(["payablesBills", search, page], () => payablesBillsFn({ page, search }))

  const payablesBills = data?.data?.data
  ...
  return (
    <CustomDiv className="flex flex-col !p-0">
      <div className="flex items-center justify-between p-2">
        <CustomInput
          type="search"
          placeholder="Search Bills Payable..."
